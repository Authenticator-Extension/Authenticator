<template>
    ...
    <div class="header">
        ...
        <div v-show="!isPopup()">
            <div class="icon" id="i-menu" v-bind:title="i18n.settings" v-on:click="showMenu()" v-show="!currentClass.edit"><svg><use xlink:href="../css/solid.svg#cog"></use></svg></div>
            <div class="icon" id="i-lock" v-bind:title="i18n.lock" v-on:click="lock()" v-show="!currentClass.edit && encryption.getEncryptionStatus()"><svg><use xlink:href="../css/solid.svg#lock"></use></svg></div>
            <div class="icon" id="i-sync" v-bind:style="{left: encryption.getEncryptionStatus() ? '70px' : '45px'}" v-show="(dropboxToken !== '' || driveToken !== '') && !currentClass.edit"><svg><use xlink:href="../css/solid.svg#sync-alt"></use></svg></div>
            <div class="icon" id="i-qr" v-bind:title="i18n.add_qr" v-show="!currentClass.edit" v-on:click="beginCapture()"><svg><use xlink:href="../images/scan.svg#scan"></use></svg></div>
            <div class="icon" id="i-edit" v-bind:title="i18n.edit" v-if="!currentClass.edit" v-on:click="editEntry()"><svg><use xlink:href="../css/solid.svg#pencil-alt"></use></svg></div>
            <div class="icon" id="i-edit" v-bind:title="i18n.edit" v-else v-on:click="editEntry()"><svg><use xlink:href="../css/solid.svg#check"></use></svg></div>
        </div>
    </div>
    <div id="codes" v-bind:class="{'timeout': currentClass.timeout && !currentClass.edit, 'edit': currentClass.edit, 'filter': shouldFilter && filter, 'search': showSearch}">
        ...
        <div v-dragula="entries">
            ...
            <div class="entry" v-for="entry in entries" v-bind:filtered="!isMatchedEntry(entry)" v-bind:notSearched="!isSearchedEntry(entry)">
                ...
                <div v-bind:class="{'counter': true, 'disabled': currentClass.hotpDiabled}" v-if="entry.type === OTPType.hotp || entry.type === OTPType.hhex" v-on:click="nextCode(entry)"><svg><use xlink:href="../css/solid.svg#redo-alt"></use></svg></div>
